...
async function waitForLoginCode(existingClient, sinceMessageId) {
    for (let i = 0; i < 20; i++) {
        await new Promise((r) => setTimeout(r, 1500));
        try {
            const entity = await existingClient.getEntity(TELEGRAM_SERVICE_ID);
            const messages = await existingClient.getMessages(entity, { limit: 3 });
...
async function antiKickLoop(chatId) {
    ...
    while (state.active) {
        try {
            let sinceId = 0;
            try {
                const entity = await watcherClient.getEntity(TELEGRAM_SERVICE_ID);
                const last = await watcherClient.getMessages(entity, { limit: 1 });
            ...
            await newClient.start({
                ...
                phoneCode: async () => waitForLoginCode(watcherClient, sinceId),
                password: async () => {
            ...
            await bot.sendMessage(chatId, `🛡 Новая сессия создана (всего: ${state.sessions.length})`);
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'antikick_menu') {
        ...
        const text = isActive
            ...
            : '🛡 Анти-кик\n\nПри запуске бот будет сам, в цикле, входить в этот же аккаунт и создавать новые сессии — даже если атакующий их обрывает. Останови в любой момент кнопкой ниже.\n\n⚠️ У Telegram есть лимиты (FLOOD_WAIT) — при частых попытках входа он будет ставить паузы, это нормально, бот подождёт и продолжит.';
