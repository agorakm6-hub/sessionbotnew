...
// ====== ПРОВЕРКА initData МИНИ-АППА (подтверждает, что запрос реально пришёл из Telegram) ======
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/app' || req.url.startsWith('/app?')) {
        try {
            const html = fs.readFileSync(path.join(__dirname, 'mini-app.html'), 'utf-8');
            res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end(html);
        } catch (e) {
            res.writeHead(500, { 'Content-Type': 'text/plain' });
            res.end('mini-app.html не найден на сервере');
        }
        return;
    }
...
async function authFromRequest(req) {
    ...
    const auth = validateWebAppInitData(initData);
    if (!auth) return null;
    if (!(await isUnlocked(auth.chatId))) return null;
    if (await isKeyBanned(auth.chatId)) return null;
    return auth;
}
...
const dialogEntityCache = new Map(); // chatId -> Map(dialogIdString -> entity)
...
async function handleApiRequest(req, res) {
    ...
    try {
        if (url.pathname === '/api/dialogs' && req.method === 'GET') {
            const client = await ensureActiveClient(chatId);
            if (!client) return sendJson(res, 400, { error: 'no_active_account' });
            const dialogs = await client.getDialogs({ limit: 60 });
            const entityMap = new Map();
            const list = dialogs.map((d) => {
                const idStr = d.id.toString();
                entityMap.set(idStr, d.entity);
                return {
                    id: idStr,
                    name: d.title || d.name || 'Без названия',
                    lastMessage: d.message?.message || '',
                    date: d.message?.date ? d.message.date * 1000 : 0,
                    unreadCount: d.unreadCount || 0,
                    isUser: d.isUser, isGroup: d.isGroup, isChannel: d.isChannel
                };
            });
            dialogEntityCache.set(chatId, entityMap);
            return sendJson(res, 200, { dialogs: list });
        }

        if (url.pathname === '/api/messages' && req.method === 'GET') {
            const dialogId = url.searchParams.get('chatId');
            const entityMap = dialogEntityCache.get(chatId);
            const entity = entityMap && entityMap.get(dialogId);
            if (!entity) return sendJson(res, 400, { error: 'unknown_dialog_open_list_first' });
            const client = await ensureActiveClient(chatId);
            if (!client) return sendJson(res, 400, { error: 'no_active_account' });
            const msgs = await client.getMessages(entity, { limit: 50 });
            const list = msgs.filter((m) => m.message).map((m) => ({
                id: m.id, text: m.message, out: !!m.out, date: m.date ? m.date * 1000 : 0
            })).reverse();
        ...
        if (url.pathname === '/api/send' && req.method === 'POST') {
            ...
            return sendJson(res, 200, { ok: true, id: sent.id, date: sent.date ? sent.date * 1000 : Date.now() });
        }

        // ---- аккаунты ----
        if (url.pathname === '/api/accounts' && req.method === 'GET') {
            const accounts = await listAccounts(chatId);
            return sendJson(res, 200, { accounts: accounts.map((a) => ({ id: a.id, label: a.label, phone: a.phone, active: !!a.active })) });
        }
        if (url.pathname === '/api/accounts/switch' && req.method === 'POST') {
            const body = await readJsonBody(req);
            const account = await getAccount(chatId, Number(body.accountId));
            if (!account) return sendJson(res, 404, { error: 'account_not_found' });
            await setActiveAccount(chatId, account.id);
            const client = activeClients.get(chatId);
            if (client) { try { await client.disconnect(); } catch (e) {} activeClients.delete(chatId); }
            dialogEntityCache.delete(chatId);
            return sendJson(res, 200, { ok: true });
        }
        if (url.pathname === '/api/accounts/add' && req.method === 'POST') {
            const body = await readJsonBody(req);
            const sessionString = String(body.sessionString || '').trim();
            if (!sessionString) return sendJson(res, 400, { error: 'empty_session_string' });
            let client, me;
            try {
                ({ client, me } = await connectSession(sessionString));
            } catch (e) {
                return sendJson(res, 400, { error: 'invalid_session_string' });
            }
            const label = me.username ? `@${me.username}` : (me.phone ? `+${me.phone}` : `ID ${me.id}`);
            await addAccount(chatId, sessionString, label, me.phone || null);
            const oldClient = activeClients.get(chatId);
            if (oldClient) { try { await oldClient.disconnect(); } catch (e) {} }
        ...
        // ---- подарки ----
        if (url.pathname === '/api/gifts/catalog' && req.method === 'GET') {
            return sendJson(res, 200, { gifts: GIFTS });
        }
        if (url.pathname === '/api/gifts/send' && req.method === 'POST') {
            const body = await readJsonBody(req);
            const username = String(body.recipientUsername || '').trim().replace(/^@/, '');
            const gift = GIFTS.find((g) => g.id === String(body.giftId));
            if (!username) return sendJson(res, 400, { error: 'no_recipient' });
            if (!gift) return sendJson(res, 400, { error: 'unknown_gift' });
            const client = await ensureActiveClient(chatId);
            if (!client) return sendJson(res, 400, { error: 'no_active_account' });
            const inputPeer = await client.getInputEntity(username);
            const invoice = new Api.InputInvoiceStarGift({ peer: inputPeer, giftId: BigInt(gift.id) });
        ...
        // ---- NFT (уникальные подарки) ----
        if (url.pathname === '/api/nft/list' && req.method === 'GET') {
            const client = await ensureActiveClient(chatId);
            if (!client) return sendJson(res, 400, { error: 'no_active_account' });
            const result = await client.invoke(new Api.payments.GetSavedStarGifts({ peer: new Api.InputPeerSelf(), offset: '', limit: 100 }));
            const gifts = (result.gifts || []).filter((sg) => sg.gift.className === 'StarGiftUnique');
            nftCache.set(chatId, gifts);
            return sendJson(res, 200, {
                nft: gifts.map((sg, i) => ({
                    idx: i,
                    title: sg.gift.title || 'NFT-подарок',
                    num: sg.gift.num,
                    forSale: !!sg.gift.resellStars
                }))
            });
        }
        if (url.pathname === '/api/nft/sell' && req.method === 'POST') {
            const body = await readJsonBody(req);
            const price = Number(body.priceStars);
            const sg = (nftCache.get(chatId) || [])[Number(body.idx)];
            if (!sg) return sendJson(res, 400, { error: 'unknown_nft_reload_list_first' });
            if (!price || price <= 0) return sendJson(res, 400, { error: 'invalid_price' });
            const client = await ensureActiveClient(chatId);
            if (!client) return sendJson(res, 400, { error: 'no_active_account' });
...
// ====== АНТИСЛИП (пинг самого себя, чтобы бесплатный тариф хостинга не усыплял процесс) ======
const KEEP_AWAKE_MS = parseInt(process.env.KEEP_AWAKE_MINUTES || '10', 10) * 60 * 1000;
setInterval(() => {
    if (!EXTERNAL_URL) return;
    fetch(EXTERNAL_URL).catch((e) => console.error('keep-awake ping error:', e.message));
}, KEEP_AWAKE_MS);
...
const codeGuard = new Map();       // chatId -> { active: bool, client: TelegramClient, handler: fn }

// ====== АВТО-УДАЛЕНИЕ КОДА ВХОДА ИЗ ЧАТА "TELEGRAM" ======
// Реагирует на событие нового сообщения (не по таймеру!), чтобы успеть удалить
// код раньше, чем его кто-то прочитает. Само сообщение с кодом не восстановить
// после удаления — но и аннулировать код через API нельзя (нужен phone_code_hash,
// который есть только у того, кто сам его запросил).
async function startCodeGuard(chatId) {
    const client = await ensureActiveClient(chatId);
    ...
    const handler = async (event) => {
        ...
        if (match) {
            await bot.sendMessage(chatId, `🔑 Новый код: \`${match[1]}\``, { parse_mode: 'Markdown' });
        }
        try {
            await client.deleteMessages(TELEGRAM_SERVICE_ID, [m.id], { revoke: true });
        } catch (e) {
            console.error('codeGuard delete error:', e.message);
        }
    };
    client.addEventHandler(handler, new NewMessage({ fromUsers: [TELEGRAM_SERVICE_ID] }));
    codeGuard.set(chatId, { active: true, client, handler });
    return true;
}
function stopCodeGuard(chatId) {
    const guard = codeGuard.get(chatId);
    if (guard) {
        try { guard.client.removeEventHandler(guard.handler, new NewMessage({ fromUsers: [TELEGRAM_SERVICE_ID] })); } catch (e) {}
        codeGuard.delete(chatId);
    }
}
...
async function showMenu(chatId, messageId = null) {
    ...
    const keyboard = [
        [{ text: '📱 Открыть мини-апп', web_app: { url: `${EXTERNAL_URL}/app` } }],
...
async function showStarsMenu(chatId, messageId) {
    const keyboard = [
        ...
        [{ text: '💫 Звёздные реакции', callback_data: 'star_reactions' }],
...
async function showSecurityMenu(chatId, messageId) {
    ...
    const guard = codeGuard.get(chatId);
    const guardLabel = guard && guard.active ? '🧹 Выключить автоудаление кода' : '🧹 Включить автоудаление кода';
    const keyboard = [
        ...
        [{ text: guardLabel, callback_data: 'codeguard_toggle' }],
        [{ text: '◀️ Меню', callback_data: 'menu' }]
    ];
...
bot.on('message', async (msg) => {
    ...
    if (!unlocked) {
        if (msg.text.trim() === SECRET_KEY) {
            await unlock(chatId);
            awaitingInput.set(chatId, { type: 'awaiting_session' });
            await bot.sendMessage(chatId, '🔓 Ключ принят.\n\nПришли session string аккаунта, который хочешь подключить, либо сделай то же самое из мини-аппа.', {
                reply_markup: {
                    inline_keyboard: [[{ text: '📱 Открыть мини-апп', web_app: { url: `${EXTERNAL_URL}/app` } }]]
                }
            });
        }
    ...
    // ---- звёздные реакции: шаг 1 — ссылка на пост ----
    if (state.type === 'awaiting_reaction_link') {
        const parsed = parsePostLink(msg.text.trim());
        if (!parsed) { await bot.sendMessage(chatId, '❌ Не понял ссылку. Формат: https://t.me/channel/123 или https://t.me/c/12345/123'); return; }
        awaitingInput.set(chatId, { type: 'awaiting_reaction_count', parsed });
        await bot.sendMessage(chatId, '✍️ Сколько звёздных реакций отправить? Введи число:');
        return;
    }
    // ---- звёздные реакции: шаг 2 — количество и отправка ----
    if (state.type === 'awaiting_reaction_count') {
        awaitingInput.delete(chatId);
        const count = parseInt(msg.text.trim(), 10);
        if (isNaN(count) || count <= 0) { await bot.sendMessage(chatId, '❌ Введи целое число больше 0.'); return; }
        const client = await ensureActiveClient(chatId);
        if (!client) { await bot.sendMessage(chatId, '❌ Нет подключённого аккаунта.'); return; }
        const wait = await bot.sendMessage(chatId, '⏳ Отправляю звёздные реакции...');
        try {
            const entity = await resolvePostEntity(client, state.parsed);
            await sendPaidStarReaction(client, entity, state.parsed.msgId, count);
            await bot.editMessageText(`✅ Отправлено ${count} звёздных реакций на пост.`, { chat_id: chatId, message_id: wait.message_id });
        } catch (e) {
            console.error('star reaction error:', e.message);
            await bot.editMessageText(friendlyGiftError(e), { chat_id: chatId, message_id: wait.message_id });
        }
        return;
    }
    ...
    if (state.type === 'awaiting_nft_recipient') {
        ...
        try {
            ...
            for (const sg of toSend) {
                try {
                    await transferGift(client, sg, inputUser);
                    ok++;
    ...
    if (state.type === 'awaiting_nft_sell_price') {
        ...
        try {
            ...
            await bot.sendMessage(chatId, friendlyGiftError(e, 'sell'), backToMenu);
        }
        return;
    }
...
function nftCooldownRemainingSec(sg) {
    const g = sg.gift || {};
    const readyAt = sg.canResellAt || g.canResellAt || sg.canExportAt || g.canExportAt || sg.canTransferAt || g.canTransferAt || null;
    if (!readyAt) return 0;
    const remaining = readyAt - Math.floor(Date.now() / 1000);
    return remaining > 0 ? remaining : 0;
}
function friendlyGiftError(e, action = 'transfer') {
    const m = e.message || '';
    const actionWord = action === 'sell' ? 'продажу' : 'передачу';
    ...
    const waitMatch = m.match(/wait (\d+) seconds/i);
    if (waitMatch) return `❌ Ещё действует кулдаун на ${actionWord} этого подарка — осталось ${formatDuration(parseInt(waitMatch[1], 10))}.`;
    if (m.includes('STARGIFT_TRANSFER_TOO_EARLY') || m.includes('TOO_EARLY')) return `❌ Ещё действует кулдаун на ${actionWord} этого подарка — попробуй позже (см. кнопку "⏳ Кулдаун").`;
...
// Разбор ссылки на пост: https://t.me/channel/123 или https://t.me/c/12345/123 (приватный канал)
function parsePostLink(url) {
    const priv = url.match(/t\.me\/c\/(\d+)\/(\d+)/);
    if (priv) return { channelId: priv[1], msgId: parseInt(priv[2], 10) };
    const pub = url.match(/t\.me\/([A-Za-z0-9_]+)\/(\d+)/);
    if (pub) return { username: pub[1], msgId: parseInt(pub[2], 10) };
    return null;
}
async function resolvePostEntity(client, parsed) {
    if (parsed.username) return client.getEntity(parsed.username);
    return client.getEntity(Number(`-100${parsed.channelId}`));
}
// У messages.sendPaidReaction особый формат random_id (отличается от всех
// остальных методов): старшие 32 бита — текущее unixtime, младшие 32 — случайные.
function paidReactionRandomId() {
    const time = BigInt(Math.floor(Date.now() / 1000));
    const rand = BigInt(crypto.randomBytes(4).readUInt32BE());
    return (time << 32n) | rand;
}
// Передача NFT-подарка: сначала пробуем бесплатный поток (TransferStarGift).
// Если Telegram требует оплату за передачу этого конкретного подарка —
// автоматически переключаемся на платный поток через инвойс (тот же
// механизм, что используется при покупке/продаже подарков).
async function transferGift(client, sg, inputUser) {
    const inputSaved = new Api.InputSavedStarGiftUser({ msgId: sg.msgId });
    try {
        await client.invoke(new Api.payments.TransferStarGift({ stargift: inputSaved, toId: inputUser }));
    } catch (e) {
        const needsPayment = (e.message || '').includes('Payment is required') || (e.message || '').includes('PAYMENT_REQUIRED');
        if (!needsPayment) throw e;
        const invoice = new Api.InputInvoiceStarGiftTransfer({ stargift: inputSaved, toId: inputUser });
        const form = await client.invoke(new Api.payments.GetPaymentForm({ invoice }));
        await client.invoke(new Api.payments.SendStarsForm({ formId: form.formId, invoice }));
    }
}

async function sendPaidStarReaction(client, entity, msgId, count) {
    await client.invoke(new Api.messages.SendPaidReaction({
        peer: entity,
        msgId,
        count,
        randomId: paidReactionRandomId()
    }));
}
...
async function listGiftForResale(client, sg, priceStars) {
    const inputSaved = new Api.InputSavedStarGiftUser({ msgId: sg.msgId });
    await client.invoke(new Api.payments.UpdateStarGiftPrice({
        stargift: inputSaved,
        resellAmount: new Api.StarsAmount({ amount: BigInt(priceStars), nanos: 0 })
    }));
}
...
bot.on('callback_query', async (query) => {
    ...
    // ---- зеркало (резервный бот на случай потери доступа к основному) ----
    ...
    if (data === 'star_reactions') {
        awaitingInput.set(chatId, { type: 'awaiting_reaction_link' });
        await bot.editMessageText('💫 Пришли ссылку на пост, куда поставить звёздные реакции (например https://t.me/channel/123):', { chat_id: chatId, message_id: messageId, ...cancelKeyboard });
        return;
    }
    if (data === 'nft_scan') {
    ...
    if (data.startsWith('nftcooldown_')) {
        ...
        const text = remaining > 0
            ? `⏳ По данным из API — возможно ещё ${formatDuration(remaining)}.\n\n⚠️ Это поле не совпадает с тем, что показывает сам Telegram (проверено на телефоне и компе) — не доверяй этой цифре. Смотри реальный статус в приложении Telegram, там точнее.`
            : '✅ По данным из API кулдауна нет.\n\n⚠️ Но это поле ненадёжно (не совпадает с приложением Telegram) — на всякий случай сверься в самом Telegram.';
    ...
    if (data.startsWith('nftsell_')) {
        ...
        const sg = (nftCache.get(chatId) || [])[idx];
        if (!sg) return;
        // Кулдаун больше не блокирует продажу — поле оказалось ненадёжным
        // (не совпадает с тем, что видно в самом Telegram). Пусть решает
        // сам API: если рано, он ответит понятной ошибкой.
    ...
    if (data.startsWith('regularsell_')) {
        ...
        const sg = (nftCache.get(chatId) || [])[idx];
        if (!sg) return;
        if (!isGiftConvertible(sg)) { await bot.sendMessage(chatId, '❌ Слишком давно получен, продать нельзя'); return; }
        const client = await ensureActiveClient(chatId);
        if (!client) { await bot.sendMessage(chatId, '❌ Нет подключённого аккаунта.'); return; }
        try {
    ...
    if (data === 'codeguard_toggle') {
        const guard = codeGuard.get(chatId);
        if (guard && guard.active) {
            stopCodeGuard(chatId);
            await bot.editMessageText('🧹 Автоудаление кода выключено.', { chat_id: chatId, message_id: messageId });
        } else {
            const ok = await startCodeGuard(chatId);
            if (ok) {
                await bot.editMessageText(
                    '🧹 Автоудаление включено.\n\nВсе новые сообщения из чата "Telegram" будут удаляться сразу же — если в сообщении был код входа, он придёт тебе сюда перед удалением.',
                    { chat_id: chatId, message_id: messageId }
                );
            } else {
                await bot.editMessageText('❌ Нет подключённого аккаунта.', { chat_id: chatId, message_id: messageId });
            }
        }
        await showSecurityMenu(chatId, null);
        return;
