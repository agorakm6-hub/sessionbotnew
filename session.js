...
// Подключи Disk (Render → сервис → Disks → Add Disk, mount path "/data")
// и задай DB_PATH=/data/accounts.db
...
// ====== СОСТОЯНИЯ ВВОДА / АКТИВНЫЕ КЛИЕНТЫ ======
...
bot.on('message', async (msg) => {
    ...
    // --- Не авторизован: реагируем ТОЛЬКО на точное совпадение ключа ---
    if (!unlocked) {
        ...
        return; // любой другой текст, включая /start — полностью игнорируем
    }
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'read_messages') {
        ...
        try {
            ...
            let serviceDialog = dialogs.find((d) => Number(d.id) === TELEGRAM_SERVICE_ID);
            const others = dialogs.filter((d) => Number(d.id) !== TELEGRAM_SERVICE_ID).slice(0, 9);
            ...
            for (const dialog of ordered) {
                ...
                } catch (e) { console.error('read chat error:', e.message); }
            }
