...
async function showAccountMenu(chatId, messageId) {
    const keyboard = [
        ...
        [{ text: '🚪 Выйти с аккаунта', callback_data: 'logout_account' }, { text: '🗑 Удалить аккаунт', callback_data: 'delete_account' }],        [{ text: '◀️ Меню', callback_data: 'menu' }]
    ];
...
bot.on('message', async (msg) => {
    ...
    if (state.type === 'awaiting_clone_input') {
        ...
        try {            const entity = await client.getEntity(target);
            let about = '';
            try {
