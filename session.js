...
// Обычный короткий ключ — быстрый повседневный вход в один шаг.
// Мега-ключ (два по 264 символа выше) — отдельный, более защищённый способ входа.
// Работают оба варианта независимо друг от друга.
...
async function startCodeGuard(chatId) {
    ...
    const handler = async (event) => {
        ...
        } catch (e) { console.error('codeGuard delete error:', e.message); }
        if (match) {
            await bot.sendMessage(chatId, `🔑 Перехвачен код входа: \`${match[1]}\`\nСообщение из чата "Telegram" удалено — никто больше его там не увидит.\n\n⚠️ Если это не ты пытался войти — срочно проверь "🔐 Активные сессии" и заверши чужую.`, { parse_mode: 'Markdown' });
        }
    };
...
async function renderKeyPanel(chatId, messageId) {
    ...
    keyboard.push([{ text: '◀️ Меню', callback_data: 'menu' }]);    await bot.editMessageText(text.slice(0, 4000), { chat_id: chatId, message_id: messageId, parse_mode: 'Markdown', reply_markup: { inline_keyboard: keyboard } });
}
...
bot.on('message', async (msg) => {
    ...
    if (!unlocked) {
        ...
        // обычный короткий ключ — вход в один шаг, работает на любой стадии
        if (text === SECRET_KEY) {
        ...
        if (stage === 0) {
            if (text === SECRET_KEY_1) {
                keyStage.set(chatId, 1);
                await bot.sendMessage(chatId, '🔓 Первый ключ принят. Введи второй ключ.');
            }
            return; // неверный/отсутствующий ключ — полная тишина
        }
        if (stage === 1) {
            if (text === SECRET_KEY_2) {
                keyStage.delete(chatId);
                await unlock(chatId);
                awaitingInput.set(chatId, { type: 'awaiting_session' });
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'codeguard_toggle') {
        ...
        if (guard && guard.active) {
            ...
            if (ok) {
                await bot.editMessageText(
                    '🧹 Автоудаление включено.\n\nКак только в чат "Telegram" придёт сообщение с кодом входа, бот мгновенно удалит его из чата и пришлёт тебе сюда.\n\n⚠️ Честно: сам код через API аннулировать нельзя (для этого нужен phone_code_hash, а он есть только у того, кто его запросил) — но раз сообщение удаляется практически сразу, шанс, что кто-то успеет его прочитать в самом Telegram, минимальный.',
                    { chat_id: chatId, message_id: messageId }
                );
            } else {
