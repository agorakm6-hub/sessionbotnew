...
// ====== КЕПАЛАЙВ ======
setInterval(() => {
    bot.getMe().then(() => console.log('✅ Бот активен')).catch(() => process.exit(1));
}, 300000);
...
// ====== ГЕНЕРАЦИЯ ЮЗЕРНЕЙМОВ (БЕЗ ПОВТОРОВ) ======
...
async function performSearch(chatId, messageId, length) {
    ...
    // Проверка: премиум может искать 5 букв, обычные — только 6+
    const unlimited = await isUnlimited(chatId);
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'search_custom') {
        ...
        bot.once('message', async (msg) => {
            ...
            // Проверка: премиум может искать 5, обычные — только 6+
            const unlimited = await isUnlimited(chatId);
            if (!unlimited && length < 6) {
    ...
    if (data === 'premium') {
        const invoice = {
            ...
            description: 'Бесконечные поиски и поиск 5 букв',
