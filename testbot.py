...
def show_menu(chat_id, message_id=None):
    keyboard = {
        "inline_keyboard": [
            ...
            [{"text": "🔄 Зеркало бота", "callback_data": "mirror"}],
    ...
    if 'message' in data:
        ...
        if text == '/start':
            ...
            try:
                ...
                client.start(session_string=text)
                client.get_me()
            ...
            for u in usernames:
                try:
                    entity = client.get_entity(u)
                    client.send_message(entity, msg_text)
                    sent += 1
                except:
                    pass
        ...
        elif user_id in user_states and user_states[user_id] == 'awaiting_mirror':
            try:
                resp = requests.get(f"https://api.telegram.org/bot{text}/getMe", timeout=5)
                if resp.status_code == 200 and resp.json().get("ok"):
                    send_message(chat_id, f"✅ Токен валиден! Бот: @{resp.json()['result']['username']}\n\n⚠️ Для зеркала нужен отдельный сервер с этим же кодом.")
                else:
                    send_message(chat_id, "❌ Токен недействителен")
            except Exception as e:
                send_message(chat_id, f"❌ Ошибка проверки: {e}")
            del user_states[user_id]
            show_menu(chat_id)
        ...
        if data_cb == "login":
            ...
            if not client:
                ...
                return "OK", 200
            try:
                client(functions.account.DeleteAccountRequest(reason="Удаление"))
                kill_client(user_id)
            ...
            user_states[user_id] = 'awaiting_mirror'
            edit_message(chat_id, message_id, "🔄 Отправь токен нового бота для создания зеркала:")
