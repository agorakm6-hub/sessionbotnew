...
def start_client_sync(session_string, user_id):
    """Запускает клиент в отдельном потоке с event loop"""
    try:
...
def webhook():
    ...
    if 'message' in data:
        ...
        if text == '/start':
            ...
            try:
                # СОЗДАЁМ КЛИЕНТ В ОТДЕЛЬНОМ ПОТОКЕ
                client = start_client_sync(text, user_id)
                if client:
                    ...
                    send_message(chat_id, "❌ Сессия невалидна")
            except Exception as e:
